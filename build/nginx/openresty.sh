...
wget https://github.com/cloudflare/zlib/archive/gcc.amd64.zip -O zlib.zip; unzip zlib.zip; rm zlib.zip
cd zlib-gcc.amd64
sed -ie 's/LDCONFIG\=\$/CFLAGS\=\"-O3\ -march=native"\nLDCONFIG\=\$\{/' configure
./configure
cd $WD
...
./configure -j$PARALLEL --user=www-data --group=www-data --with-file-aio --with-ipv6 --with-pcre-jit --with-http_realip_module --with-http_stub_status_module --with-http_v2_module --with-zlib=../zlib-gcc.amd64 --add-module=../ngx_brotli-master
